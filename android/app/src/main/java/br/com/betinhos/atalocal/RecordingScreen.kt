...
fun RecordingScreen(startedAt: Long, paused: Boolean, pauseStartedAt: Long, pausedDurationMs: Long, level: Float, onTogglePause: () -> Unit, onStop: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(28.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
        Text(if (paused) "PAUSADO" else "GRAVANDO", style = MaterialTheme.typography.labelLarge, color = if (paused) MaterialTheme.colorScheme.tertiary else MaterialTheme.colorScheme.primary)
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            OutlinedButton(onClick = onTogglePause) { Text(if (paused) "Continuar" else "Pausar") }
